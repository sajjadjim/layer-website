...
const BookingLayer = () => {
  ...
  const handleLayerRemoveAppointment = (id) => {
    const updatedAppointments = layerListAppointment.filter(
      (layer) => layer.licenseNumber !== id
    );
      ...
      <div>
        {layerListAppointment.length === 0 ? (
          ...
          <div className="my-10 grid justify-center">
            <Rechart layerListAppointment={layerListAppointment}></Rechart>
          </div>
        )}
      </div>
      <div>
        ...
        {layerListAppointment.length === 0 ? (
          <div className="mb-50">
            <p className="text-center text-xl text-gray-500 mt-20 mb-20">
              No appointments available
            </p>
            <div className="flex  justify-center">
              <Link
                to="/"
                className="bg-blue-500 rounded-xl py-2 px-3 text-white font-semibold"
              >
                Book an Appointment{" "}
              </Link>
            </div>
          ...
          <div>
            {layerListAppointment.map((layer, index) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    handleLayerRemoveAppointment(layer.licenseNumber)
                  }
